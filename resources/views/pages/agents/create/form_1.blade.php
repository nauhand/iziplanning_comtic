...
<div class="box-body form-block" >
    ...
    <div class="tab">
        ...
        <div class="box-body">
            ...
            <div class="form-group nom @error('nom')  has-error @enderror">
                <label>Nom (*)</label>
                <input name="nom" type="text" class="form-control"  placeholder="Entrer le nom" value="{{old('nom') ?: $agent->nom}}" required>
                @error('nom')
                    <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> {{$message}}</label>
                @enderror
            </div>
            <div class="form-group datenaissance @error('datenaissance')  has-error @enderror">
                ...
                <input type="date" name="datenaissance" class="form-control pull-right" id="datepicker" min="{{Carbon::now()->addYear(-100)->toDateString()}}" max="{{Carbon::now()->addYear(-18)->toDateString()}}" value="{{old('datenaissance') ?: $agent->datenaissance}}" required>
                </div>
                <!-- /.input group -->
                @error('datenaissance')
                    <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> {{$message}}</label>
            ...
            <div class="col-md-6">
                ...
                <div class="form-group prenoms @error('prenoms')  has-error @enderror">
                    <label>Prénoms (*)</label>
                    <input name="prenoms" type="text" class="form-control" placeholder="Entrer le(s) Prénom(s)" value="{{old('prenoms') ?: $agent->prenoms}}" required>
                    @error('prenoms')
                    <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> {{$message}}</label>
                    @enderror
                </div>
    ...
    <div style="overflow:auto;margin-right: 26px">
        <div style="float:right;">
            <button type="button" class="btn btn-flat btn-primary suivant" onclick="submitForm('regForm')" data-route="{{ route('agent.addVerification') }}" data-type="form_1">Etape Suivante</button>
        </div>
    </div>
