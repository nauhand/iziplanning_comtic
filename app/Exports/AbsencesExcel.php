...
class AbsencesExcel implements FromCollection, ShouldAutoSize, WithEvents, WithDrawings, WithHeadings
{
    // private $agent;
    private $conges;
    private $donneeExcel = [];
    ...
    {   
      $this->conges  =  Conge::where('date_debut', '>=', Carbon::today())
    ...
    */
    public function collection()
    {
        ...
        foreach($this->conges as $key => $conge) {
            $this->donneeExcel[] = array(
                'N°' =>  $key+1 ,
                'NOM ET PRENOMS SITE'=> $conge->agent->nom . ' '. $conge->agent->prenoms ,
                'TEL DE L\'AGENT'   => $conge->agent->numeromobile  ,
                'SITUATION GÉO'  => $conge->agent->adressegeo  ,
                'VILLE'  => $conge->agent->ville  ,
                'TYPE D\'ABSENCE'    => mb_convert_case($conge->typeconge, MB_CASE_TITLE, "UTF-8"),
                'DATE DE DÉBUT' =>  ucfirst(\Carbon\Carbon::parse($conge->date_debut)->locale('fr_FR')->isoFormat('ddd DD, MM YYYY')),
                'DATE DE FIN' => ucfirst(\Carbon\Carbon::parse($conge->date_fin)->locale('fr_FR')->isoFormat('ddd DD, MM YYYY'))  ,
                'NOMBRE DE JOUR ' => \Carbon\Carbon::parse($conge->date_debut)->diffInDays(\Carbon\Carbon::parse($conge->date_fin))  ,
            );

        }
                
        return collect( $this->donneeExcel);
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $cellRange = 'A1:H' . ( $this->conges->count() + 5 );
                ...
                $event->sheet->setCellValue(sprintf('A9'),'LISTE DES AGENTS ABSENT');
    ...
    {
        return array('N°', 'NOM ET PRENOMS', 'TEL DE L\'AGENT', 'SITUATION GÉO', 'VILLE', 'TYPE D\'ABSENCE', 'DATE DE DÉBUT', 'DATE DE FIN', 'NOMBRE DE JOUR ');
    }
