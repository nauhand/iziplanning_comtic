...
class PlanninsgDefControllerExcel implements FromCollection, ShouldAutoSize, WithEvents, WithDrawings, WithHeadings
{
    // private $agent;
    private $plannings;
    private $total ;
    ...
    public function collection()
    {
        ...
        foreach ($this->plannings as $key => $planning)
        {
            ...
            $donneeExcel[] = array(
                ...
                'HEURES DE JOUR'=> BlackshFonctions::format($heures_total_hours . ':'.$minutes_jours.':00') ,
                'HEURES DE NUIT'=> BlackshFonctions::format($heure_total_nuit . ':'.$minutes_nuit.':00') ,
                'TOTAL HEURES'  => $total . ':'.$minutes.':00'
            );
        }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $event->sheet->setCellValue('G3','Heures totales : '.BlackshFonctions::format($total. ':'.$minutes.':00'));
                ...
                $event->sheet->setCellValue(sprintf('A9'),'PLANNING  DEFINITIF DE '. $this->plannings[0]->agent->nom .' '. $this->plannings[0]->agent->prenoms);
    ...
    {
        return array('DATE', 'SITE', 'DÉBUT SERVICE', 'FIN SERVICE', 'HEURES PAUSE','HEURES DE NUIT', 'HEURES DE NUIT' , 'TOTAL HEURES');
    }
